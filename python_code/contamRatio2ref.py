...
def contam_ratio(n50, nmapped_source, nmapped_sink, KS_THRESHOLD=0.001, KS_FLAT_THRESHOLD=0.01, P_RARIO=0.05) :
    ...
    for i in range(n50):
        ...
        if contig not in nmapped_sink:  # this means n_mapped_reads == 0
    ...
    if nave != 0:
        average /= nave
        var = math.sqrt((ave2-2.*average*ave+average*average*aven)/nave)/average
    else:
        var=0.
    ...
    for i in range (ns):
        ...
        logging.info(mssg)
        if nmapped_sink[contig].similar < KS_THRESHOLD and pfe > P_RARIO:
            nsink+=src
            nsource+=snk
    if nsource + nsink == 0:
        return (nsink, nsource, var, ns, first_none_match, tsnk, tsrc)
...
with open(contamfile,'w') as outf:
    for lib_fr in nmap_cross:
        ...
        if tsnk != 0 and tot_mapped != 0:
            outf.write('\t'.join([lib_to, lib_fr, str(nsnk), str(nsrc), str(ns), str(first_none_match), str(tot_mapped), str(var), str(tsnk), str(tsrc)]))
            outf.write('\n')
