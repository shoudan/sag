...
#pick_random_region_test(contigs_by_lib_name)
...
contamCtg={}
contamCtgSrc={}
find_source_and_sink(contigs_by_lib_name, lst_contig_overlap, profile_fw, profile_rv)
org_by_id=overlap (contigs_by_lib_name, lst_contig_overlap)
...
for lib in contamCtg:
    for ctg in contamCtg[lib]:
        #if ctg in contamCtgSrc:
        #    print contamCtgSrc[ctg], ctg
        
        if ctg not in contamCtgSrc or contamCtgSrc[ctg] not in pairLibSameOrg:
            #if ctg in contamCtgSrc:
            #    print 'again', contamCtgSrc[ctg]
            print ctg
'''
