...
def contamCtgByHighRatio(contigs_by_lib_name, profile_fw, profile_rv, contamCtg, threshold=10.):
...
#                if 2*snk > tot_num_reads_snk and src/float(snk) > max_ratio:
                if (3*snk > tot_num_reads_snk and src/float(snk) > max_ratio and
                    ks_overlap_only(profile_fw[(lib_snk, snkCtg)], profile_rv[(lib_snk, snkCtg)], profile_fw[(lib_src, snkCtg)], profile_rv[(lib_src, snkCtg)])[0] > 0.0001):
...
#            if contig_len(snkCtg) < 500 or contigs_by_lib_name[lib_snk][snkCtg].nreads < 20 or (max_ratio > threshold and ks_overlap_only(profile_fw[(lib_snk, snkCtg)], profile_rv[(lib_snk, snkCtg)], profile_fw[(lib_max, snkCtg)], profile_rv[(lib_max, snkCtg)])[0] > 0.0001):
            if max_ratio > threshold:
                ...
                max_src_read=0
                for val in tot_num_reads_src.itervalues():
                    if val > max_src_read:
                        max_src_read = val
                if max_src_read / float(tot_num_reads_snk) > threshold:
                # test if profile is similar to the combined profile of several source
                    prof_fw=[]
                    prof_rv=[]
                    for lib in tot_num_reads_src:
# contam14
#                        if tot_num_reads_src[lib]*500 > max_src_read:
#contam15
                        if tot_num_reads_src[lib]*100 > max_src_read:
                            prof_fw += profile_fw[(lib, snkCtg)]
                            prof_rv += profile_rv[(lib, snkCtg)]
                    #print "before:", prof_fw
                    prof_fw = sorted(prof_fw, key=lambda x: x[0])
                    #print "after:", prof_fw
                    prof_rv = sorted(prof_rv, key=lambda x: x[0])
                    if ks_overlap_only(profile_fw[(lib_snk, snkCtg)], profile_rv[(lib_snk, snkCtg)], prof_fw, prof_rv)[0] > 0.0001:
                        store_contam(lib_snk, snkCtg, contamCtg, contigs_by_lib_name)
'''
                    else:
