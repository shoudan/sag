...
def contam_contig(nmapped_sink, nmapped_source, KS_THRESHOLD=0.001, Contam_max=0.05) :
    """
    ...
    if  float(nmapped_sink[1])/nmapped_source[lib].n_mapped_reads > Contam_max:
        # not a contam
        return 0
    return nmapped_sink[1]
...
flog=lib_this+'.log'
contigfile=lib_this+'-contig-num.txt'
...
with open(contigfile,'w') as outf:
    for i in range(len(nmap_self)):
        ...
        n_contam = contam_contig(nmap_self[i], nmap_cross[contig])
        if n_contam != 0:
            outf.write(contig)
            outf.write('\t')
            outf.write(str(n_contam))
            outf.write('\n')
            outf.flush()
