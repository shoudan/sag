...
def find_source_and_sink(contigs_by_lib_name, lst_contig_overlap, profile_fw, profile_rv, pval_threshold=1e-4):
...
#        print snk, srcCtg_of_snkCtg[snk]
    ...
    print '\t'.join(['sink_lib', 'source_lib', 'pval_contam', 'ratio_sink_contig', 'variance_sink_contig', 'sink_reads_mapped_to_sink_contig', 'source_reads_mapped_to_sink_contig', 'ratio_source_contig', 'variance_source_contig', 'sink_reads_mapped_to_source_contig', 'source_reads_mapped_to_source_contig'])
    for ctgp in pvalue:
#        if pvalue[ctgp] > pval_threshold:
#            continue
        output=[ctgp[1], ctgp[0], pvalue[ctgp], contamRatio_from_snkCtg[ctgp], variance_from_snk[ctgp], snkRead_to_snkCtg[ctgp], srcRead_to_snkCtg[ctgp], contamRatio_from_srcCtg[ctgp], variance_from_src[ctgp], snkRead_to_srcCtg[ctgp], srcRead_to_srcCtg[ctgp]]
...
def find_sourceContig(contigs_by_lib_name, lst_contig_overlap, profile_fw, profile_rv, SrcSnk_ratio_threshold=20.):
    '''
...
#                if lib_snk == lib_src or (snkCtg, lib_src) not in linked_ctg or (lib_src, snkCtg) not in profile_fw or (lib_src, snkCtg) not in profile_rv:
...
#                print snkCtg, lib_src, nreads, 'max', max_src_nread
                if nreads > max_src_nread:
                    snk, src = num_overlapping_reads (profile_fw[(lib_snk, snkCtg)], profile_rv[(lib_snk, snkCtg)],
                                                      profile_fw[(lib_src, snkCtg)], profile_rv[(lib_src, snkCtg)])
                    if snk == 0 or src/float(snk) < SrcSnk_ratio_threshold:
                        continue
                    
                    # set contam, this is first pass determination and is used in overlap
...
def prob_sink_source(data_maxmin, random_maxmin, contamRatio, min_coverage=1.):
    '''
    ...
    return pvalues for each (src_lib, snk_lib) based on fisher exact test, on
    '''
    pval_maxmin={}
    for pair in data_maxmin:
        if pair not in contamRatio or contamRatio[pair]==0:
            continue
        cutoff=min_coverage/contamRatio[pair]
        ...
        # print contingency_table to log
    return pval_maxmin
...
# combining forward and reverse mapped profiles by source library
    ...
    for k in snkCtg_by_srcLib:
        ...
        for _ in range(10*n_monte_carlo):
...
#        R, pval= stats.fisher_exact(contingency_table, 'greater')
#        print k, contingency_table, pval
